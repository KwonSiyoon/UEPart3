...
void AABCharacterPlayer::SetDead()
{
	...
	// 5초 후에 앞서 비활성화 했던 항목 복구.
	GetWorld()->GetTimerManager().SetTimer(DeadTimerHandle, this, &AABCharacterPlayer::ResetPlayer, 5.0f, false);
...
void AABCharacterPlayer::Attack()
{
	...
	if (bCanAttack)
	{
		...
		if (!HasAuthority())
		{
			...
			// 고려해야할 것.
			// 캐릭터 무브먼트 설정이나 공격 시간 등은 클라이언트가 악의적으로 변경(변조)할 수 있다는 사실을 가정해야 한다.
			//FTimerHandle Handle;
			GetWorld()->GetTimerManager().SetTimer(AttackTimerHandle,this, &AABCharacterPlayer::ResetAttack, AttackTime, false);
...
void AABCharacterPlayer::AttackHitCheck()
{
	...
	if(IsLocallyControlled())
	{
		...
		// 공격 판정 타이밍을 저장.
		float HitChecktTime = GetWorld()->GetGameState()->GetServerWorldTimeSeconds();
		...
		if (!HasAuthority())
		{
			...
			if (HitDetected)
			{
				...
				ServerRPCNotifyHit(OutHitResult, HitChecktTime);
			}
			else
			{
				// 충돌 검출이 안된 경우에도 서버에 전송.
				ServerRPCNotifyMiss(Start, End, Forward, HitChecktTime);
			}
		}
		// 서버.
		else
...
void AABCharacterPlayer::ServerRPCAttack_Implementation(float AttackStartTime)
{
	...
	// 아래의 타이머가 동작하지 않을 수 있기 때문에 최소한의 시간 값을 보정.
	AttackTimeDifference = FMath::Clamp(AttackTimeDifference, 0.0f, AttackTime - 0.01f);

	// 타이머 설정.
	GetWorld()->GetTimerManager().SetTimer(AttackTimerHandle, this, &AABCharacterPlayer::ResetAttack, AttackTime - AttackTimeDifference, false);
	...
	for (auto* PlayerController : TActorRange<APlayerController>(GetWorld()))
	{
		...
		{
			AB_LOG(LogABNetwork, Log, TEXT("%s"), TEXT("IsLocalController Begin"));
...
void AABCharacterPlayer::MulticastRPCAttack_Implementation()
{
	...
	//AB_LOG(LogABNetwork, Log, TEXT("%s"), TEXT("몽타주 시작"));
	//
	//UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance();
	//AnimInstance->Montage_Play(ComboActionMontage);
	//AB_LOG(LogABNetwork, Log, TEXT("%s"), TEXT("몽타주 끝"));
}

bool AABCharacterPlayer::ServerRPCNotifyHit_Validate(const FHitResult& HitResult, float HitCheckTime)
{
	// 클라이언트로부터 공격 관련 시간 값이 전달 안됐으면, 검증 안함.
	if (LastAttackStartTime == 0.0f)
	{
		return true;
	}
	...
	// 공격을 시작한 후 공격을 판전한 시간 사이에 걸린 시간이
	// 공격 판정 최소 기준을 넘어가는 지 확인.
...
void AABCharacterPlayer::ServerRPCNotifyHit_Implementation(const FHitResult& HitResult, float HitCheckTime)
{
	...
	if (IsValid(HitActor))
	{
		...
		// 클라이언트가 보낸 정보를 기반으로,
		// 맞은 위치와 맞은 액터 사이의 거리가 공격가능 거리보다 작은지 비교.
